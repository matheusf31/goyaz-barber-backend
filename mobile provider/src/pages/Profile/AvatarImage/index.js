...
import { Container, ImageButton, ImageText, Image } from './styles';

const AvatarImage = ({ onChangeAvatar }) => {
  const [avatar, setAvatar] = useState(null);

  const handleSelectImage = useCallback(data => {
    if (data.didCancel) {
      return;
    }
    if (data.error) {
  ...
  return (
    <Container>
      ...
      >
        <ImageText>Escolher imagem</ImageText>
      </ImageButton>

      <ImageButton style={{ marginTop: 10 }} onPress={handleImageUpload}>
        <ImageText>Enviar imagem</ImageText>
      </ImageButton>
    </Container>
  );
};
